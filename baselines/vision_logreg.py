#!/usr/bin/env python3
"""Generic vision encoder + Logistic Regression baseline for ReFrameJudge.

This script is intended for DINOv3/DINOv2-style image encoders from Hugging Face.
It extracts frozen image embeddings, builds pairwise features, and trains a
binary logistic regression classifier for win/lose prediction.
"""
...
def pool_outputs(outputs, pooling):
...
@torch.no_grad()
def extract_embeddings(paths, project_root, model, processor, device, batch_size, embedding_map, pooling):
    missing = [path for path in paths if path not in embedding_map]
    if not missing:
        return embedding_map

    model.eval()
    for start in tqdm(range(0, len(missing), batch_size), desc="Extract vision embeddings"):
        ...
        features = pool_outputs(outputs, pooling)
...
def main():
    ...
    parser.add_argument("--cache", type=Path, default=Path("data/cache/vision_embeddings_fcdb_5k.npz"))
    parser.add_argument(
        "--model-name",
    ...
    print(f"Loading vision encoder: {args.model_name}")
    processor = AutoImageProcessor.from_pretrained(args.model_name, cache_dir=args.hf_cache_dir)
    encoder = AutoModel.from_pretrained(args.model_name, cache_dir=args.hf_cache_dir).to(device)
